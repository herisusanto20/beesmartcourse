...
<nav class="navbar">
    ...
    $query = "SELECT DISTINCT nama_kursus FROM kursusbeesmart";
    ...
    if (isset($_GET['nama_kursus'])) {
        $nama_gelombang = $_GET['nama_kursus'];
        
        // Mendapatkan data gelombang berdasarkan nama_gelombang
        $query = "SELECT * FROM kursusbeesmart WHERE nama_kursus='$nama_gelombang'";
        $result = mysqli_query($conn, $query);
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                $nama_gelombang = $_POST['nama_kursus'];
                $kuota = $_POST['penjelasan'];
                $periode = $_POST['tingkatan'];

                // Memperbarui data gelombang di database
                $update_query = "UPDATE kursusbeesmart SET penjelasan='$kuota', tingkatan='$periode' WHERE nama_kursus='$nama_gelombang'";
                if (mysqli_query($conn, $update_query)) {
                    echo "Data deskripsi berhasil diperbarui.";
                } else {
                    echo "Gagal memperbarui deskripsi: " . mysqli_error($conn);
                }
            }

            echo "<h2>Edit Gelombang Registrasi</h2>
                <form method='post' action=''>
                    <input type='hidden' name='nama_kursus' value='".$row['nama_kursus']."'>
                    <label for='penjelasan'>penjelasan:</label>
                    <input type='text' id='penjelasan' name='penjelasan' value='".$row['penjelasan']."' required><br><br>
                    <label for='tingkatan'>Tingkatan:</label>
                    <input type='text' id='tingkatan' name='tingkatan' value='".$row['tingkatan']."' required><br><br>
