...
<body>
  ...
  $query = "SELECT kursus, jenis_kursus, kuota FROM tb_kuota WHERE tabel = 'tb_sd'";
...
foreach ($data_kuota as $kursus => $kuota) {
  ...
  if ($kuota['Reguler'] > 0) {
    echo '<a href="formsd.php?kursus=' . $kursus . '"><button type="button" class="button">Daftar Reguler</button></a>';
  } else {
  ...
  // Periksa kuota Privat, jika 0 maka nonaktifkan tombol "Daftar Privat"
  if ($kuota['Online'] > 0) {
    echo '<a href="formsd2.php?kursus=' . $kursus . '"><button type="button" class="button">Daftar Online</button></a>';
  } else {
