...
?>
    <div class="container mt-4">
        ...
        <form action="laporan_datasd.php?cetak=1" method="get">
...
if(isset($_GET['cetak'])) {
    ...
    header("Content-Disposition: attachment; filename=laporan_datasd.xls");
}

                        //jika tanggal dari dan tanggal ke ada maka
                        if(isset($_GET['dari']) && isset($_GET['ke'])){
                            // tampilkan data yang sesuai dengan range tanggal yang dicari 
                            $data = mysqli_query($conn, "SELECT * FROM tb_sd WHERE tanggalsd BETWEEN '".$_GET['dari']."' AND '".$_GET['ke']."' ORDER BY tanggalsd ASC");
                        }else{
                            //jika tidak ada tanggal dari dan tanggal ke maka tampilkan seluruh data
                            $data = mysqli_query($conn, "SELECT * FROM tb_sd ORDER BY tanggalsd ASC");
                        }
        ...
        <a href="laporan_datasd.php?cetak=1&dari=<?php echo $_GET['dari']; ?>&ke=<?php echo $_GET['ke']; ?>"><button class="buttonn">Download</button></a>
